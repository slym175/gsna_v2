...
{
    echo intval(get_post_meta( get_the_ID(  ), 'class_fee', true ))." %";
}
...
function tutorUncompletedProfileMessage($user_id)
{
    if(is_user_logged_in()) {
        if(!isTutorCompletedProfile($user_id)) {
            printf('<div class="gs_notice"><a href="%1$s">%2$s</a> %3$s</div>',
                wc_customer_edit_account_url(),
                __('Cập nhật thông tin', GS_TEXTDOMAIN),
                __('của bạn trước khi nhận lớp *', GS_TEXTDOMAIN)
            );
        }
    }
}
...
function getClassroomAddressIframe($class_ID)
{
    ...
    printf('<iframe frameborder="0" width="600px" height="600px" src="https://www.google.com/maps/embed/v1/place?key=%1$s&q=%2$s&zoom=14"></iframe>',
...
function getSubjectNoParent()
{
    $terms = get_terms( 'class_subject', array(
        'hide_empty' => false,
    ) );
    return $terms;
}
    ...
    foreach($cities[$pro_key] as $key => $district) {
        printf('<div>
                    <div class="form-check mb-2">
                        <input type="checkbox" name="filter_district[]" value="%1$s" id="district%2$d">
                        <label for="district%3$d" class="checkmark">%4$s</label>
                    </div>
                </div>',
            $district,
            $key,
